...
def evaluate_config(config_name, segmentor, samples, use_sam3=False, threshold=0.1):
    """评估单个配置"""
    print(f"\n评估配置: {config_name}")
    print("-" * 60)
    ...
    for i, sample in enumerate(samples[:30]):  # 评估前30个样本
        ...
        try:
            if use_sam3:
                # 使用SAM3精细化分割
                result = segmentor.segment_with_sam3(
                    image, 
                    threshold=threshold,
                    return_mask=True
                )
...
def main():
    ...
    segmentor_radio = OpenVocabSegmentor(
        ...
        adaptor_names=["siglip2-g"],  # SigLIP2 only
        device="cuda",
        input_size=896,
    )
    ...
    # 加载分割器 - RADIO + SAM3
    print(f"\n{'=' * 80}")
    print("加载分割器 (RADIO + SAM3)...")
    segmentor_sam3 = OpenVocabSegmentor(
        checkpoint_path=config["cloud"]["radio"]["checkpoint"],
        radio_code_dir=config["cloud"]["radio"]["code_dir"],
        siglip2_dir=config["cloud"]["radio"]["siglip2_dir"],
        adaptor_names=["siglip2-g", "sam"],  # SigLIP2 + SAM
        device="cuda",
        input_size=896,
    )
    ...
    print(f"  Precision: {sam3_metrics['precision']:.3f}")
    print(f"  Recall: {sam3_metrics['recall']:.3f}")
    print(f"  F1: {sam3_metrics['f1']:.3f}")
    
    iou_improvement = (sam3_metrics['iou'] - baseline_metrics['iou']) / baseline_metrics['iou'] * 100
    precision_improvement = (sam3_metrics['precision'] - baseline_metrics['precision']) / baseline_metrics['precision'] * 100
    
    f1_improvement = (sam3_metrics['f1'] - baseline_metrics['f1']) / baseline_metrics['f1'] * 100
