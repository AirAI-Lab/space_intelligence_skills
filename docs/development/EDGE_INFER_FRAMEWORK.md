...
- [x] 推理性能优化 (TensorRT) — CUDA 预处理 + 异步输出管道 + per-class 检测解析，~120ms → ~25ms
- [x] 云边通讯稳定性 — 修复 MQTT loop 冲突导致 segfault，自动重连正常
- [x] 前端UI优化
- [x] 云端推理服务 (C-RADIOv4) — 零样本语义分割，裸土/扬尘/积水检测
- [ ] 插件热加载优化

### 3.2 中优先级

- [x] 多模型并行推理 — 已支持多模型独立推理 + 异步输出
- [ ] 边缘设备资源监控
- [ ] 训练任务队列
- [ ] 数据增强Pipeline
