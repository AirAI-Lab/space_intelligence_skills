...
| **训练服务** | 数据集自动下载/解压 | ✅ 完成 |
| **训练服务** | 模型自动上传到 S3 | ✅ 完成 |
| **MQTT 集成** | 边缘设备状态上报处理 | ✅ 完成 |
| **前端 UI** | 实时训练进度图表 | ⏳ 待实现 |
| **OTA 升级** | 边缘设备热更新 | ✅ 已联调验证 |
...
| VERIFYING | 验证模型文件 | 正常流程 |
| CONVERTING | ONNX 转 Engine 中 | TensorRT 转换可能需要较长时间 |
| APPLYING | 应用模型更新 | 最后阶段 |
...
- **0-25%**：下载模型文件
- **25-30%**：验证模型文件
- **30-90%**：ONNX 转 Engine（TensorRT 转换）
- **90-100%**：应用模型更新

### 替换模型功能

当 OTA 升级进度达到 100% 后，可以点击"替换模型"按钮触发热加载。此操作会：
1. 停止边缘设备上的推理模块
2. 重新加载新的 Engine 文件
3. 重启推理模块

**注意**：替换模型操作需要边缘端设备支持模型热加载功能，且设备必须保持在线状态。
...
device/{device_id}/model/reload      -- 模型热加载指令
...
**主题**: `device/{device_id}/model/reload`

**消息格式**:
```json
{
  "task_id": "OTA202602061533371813",
  "task_name": "河道检测",
  "model_id": "M_JOB202602011236009605",
  "model_display_name": "河道检测",
  "model_version": "33371813",
  "engine_path": "/home/nvidia/edge_infer/models/河道检测_33371813.engine",
  "input_width": 640,
  "input_height": 640
}
```
...
当前版本：v1.0.1 (2026-02-06)

### 更新日志

#### v1.0.1 (2026-02-06)
- **OTA 升级功能增强**
  - 修复模型文件路径不一致问题（使用 task_id 后8位作为版本标识）
  - 添加"替换模型"功能，支持触发热加载
  - 优化 MQTT 消息处理，支持更精确的进度跟踪
  - 添加部署记录清空确认机制（二次确认 + 输入 CONFIRM）
- **前端改进**
  - 部署记录清空功能增强，要求二次确认
  - 只能清除已完成、失败、已回滚的记录，不能清除正在部署中的任务
- **后端改进**
  - MQTT 连接稳定性改进
  - OTA 任务进度跟踪优化
