...
def main():
    ...
    # 检查模型文件
    checkpoint = Path('/app/water_inspection/models/C-RADIOv4-H/c-radio_v4-h_half.pth.tar')
    radio_code = Path('/app/water_inspection/models/NVlabs_RADIO')
    siglip2 = Path('/app/water_inspection/models/siglip2-giant-opt-patch16-384')
            ...
            try:
                ...
                if pred_mask is not None:
                    ...
                    if i < 10:
                        ...
                        print(f"  {status} {sample['image']}: IoU={iou:.3f}")
    ...
    if radio_results and sam3_results:
        improvement = (sam3_results['avg_iou'] - radio_results['avg_iou']) / radio_results['avg_iou'] * 100
        ...
        output_dir = Path('/app/water_inspection/outputs')
        ...
        print("=" * 80)
        print(f"\nRADIO only:")
        print(f"  平均 IoU: {radio_results['avg_iou']:.3f}")
        print(f"\nRADIO + SAM3:")
        print(f"  平均 IoU: {sam3_results['avg_iou']:.3f}")
        print(f"\nIoU 提升: {improvement:+.1f}%")
        print(f"\n✅ 结果已保存: {output_file}")
