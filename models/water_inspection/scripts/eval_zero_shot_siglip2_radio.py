...
# -*- coding: utf-8 -*-
"""
SigLIP2 + RADIO 零样本水质异常评估脚本

功能:
1. 使用正负提示词执行零样本分类与分割
2. 在 data/datasets 上输出分类与分割指标
3. 生成可视化结果与 JSON 报告
"""
...
def resolve_model_paths(project_root: Path) -> Tuple[Path, Path, Path]:
    ...
    if checkpoint is None or radio_code is None or siglip_dir is None:
        raise FileNotFoundError(
            "无法定位 C-RADIOv4 或 SigLIP2 本地模型目录，请检查 models 目录是否完整。"
        )
...
def evaluate(
    ...
    for idx, sample in enumerate(samples, 1):
        ...
        if segments:
            best = max(segments.values(), key=lambda x: x.score)
            pred_class = best.class_name
            pred_mask = best.mask if best.mask is not None else pred_mask
...
def write_markdown_report(result: dict, output_md: Path) -> None:
    summary = result["summary"]
    metrics = result["metrics"]

    lines = []
    lines.append("# SigLIP2 + RADIO 零样本评估报告")
    lines.append("")
    lines.append("## 总览")
    ...
    lines.append(f"- 可视化样本数: {summary['visualizations_saved']}")
    lines.append("")
    lines.append("## 异常类别指标")
    for cls_name, m in metrics.items():
        lines.append(
            f"- {cls_name}: P={m['precision']:.2%}, R={m['recall']:.2%}, F1={m['f1']:.2%}, mIoU={m['miou']:.3f}, TP/FP/FN={m['tp']}/{m['fp']}/{m['fn']}"
        )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="SigLIP2 + RADIO 零样本水质异常评估")
    ...
    parser.add_argument("--output-dir", type=str, default=str(PROJECT_ROOT / "outputs" / "zero_shot_eval"))
    ...
    result_json = output_dir / "zero_shot_eval_results.json"
    result_md = output_dir / "zero_shot_eval_report.md"

    with open(result_json, "w", encoding="utf-8") as f:
        json.dump(result, f, indent=2, ensure_ascii=False)

    write_markdown_report(result, result_md)
