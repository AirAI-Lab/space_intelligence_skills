...
# -*- coding: utf-8 -*-
"""
园区监测 RADIO 零样本检测 - 端到端验证脚本 v2.0

验证数据:
  - fire_lane/  (消防通道堵塞, 15张)
  - climing/    (翻越栏杆, 17张)
...
# fire_lane: GT用 class 0 (消防通道区域), 对应 fire_lane_blocked
# climing: GT用 class 1 (攀爬), 对应 fence_climbing
DATASET_TO_RADIO = {
    "fire_lane": "fire_lane_blocked",
    "climing": "fence_climbing",
}
...
# GT 使用的标签类别 ID
GT_CLASS_ID = {
    "fire_lane": 0,    # class 0 = 消防通道区域
    "climing": 1,      # class 1 = 攀爬
}

# 每个类别的可视化颜色 (BGR)
CLASS_COLORS = {
    "fire_lane_blocked": (0, 0, 255),      # 红色
    "fence_climbing": (0, 100, 255),        # 橙色
    "exit_blocked": (0, 200, 255),          # 黄橙
}
...
def main():
    ...
    parser = argparse.ArgumentParser(description="园区监测 RADIO 零样本检测验证 v2.0")
    ...
    radio_classes = ["fire_lane_blocked", "fence_climbing"]
    prompts_config = build_prompts(classes_config, radio_classes)

    print("\n" + "=" * 70)
    print("园区监测 RADIO 零样本检测 - 端到端验证 v2.0")
    print("=" * 70)
    print(f"  阈值: {args.threshold}, 最小面积: {args.min_area}")
    print(f"  检测类别: {radio_classes}")
    ...
    for dataset_name, radio_class in DATASET_TO_RADIO.items():
        ...
        if not img_dir.exists():
            logger.warning(f"数据目录不存在: {img_dir}")
            continue
        ...
        for idx, img_path in enumerate(images):
            ...
            # GT bbox: 取 GT class_id 对应的第一个标注
            ...
            if lbl_path.exists():
                ...
                for line in lines:
                    if not line.strip():
                        continue
                    parts = line.split()
                    if len(parts) >= 5 and int(parts[0]) == gt_class_id:
            ...
            print(f"  [{status}] {img_path.name[:50]:<51} "
                  f"GT={DATASET_ZH[dataset_name]:<6} Pred={pred_cn:<8} "
