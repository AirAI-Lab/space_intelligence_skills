...
> **版本**: v4.0 | **更新**: 2026-04-23
> **对齐仓库**: `edge_infer_cloud`（云边协同平台，Python）+ `edge_infer`（边缘推理平台，C++）
>
> **v4.0 核心更新**: 三大场景全面启用 RADIO 零样本检测 (施工77.70%, 园区87.50%, 水利84%)
...
│   │   │   └── _archive_v7/                       # 历史脚本归档
│   │   └── data/                             # 数据集
│   ├── construction_safety/                # 施工安全场景
│   │   ├── configs/construction_safety.yaml # v2.0 统一配置 (YOLO + RADIO)
│   │   ├── models/unified.py               # 统一检测入口
│   │   └── scripts/eval_radio_zero_shot.py  # RADIO 零样本评估
│   ├── park_monitoring/                    # 园区监测场景
│   │   ├── configs/park_monitoring.yaml    # v2.0 统一配置 (YOLO + RADIO)
│   │   ├── models/unified.py               # 统一检测入口
│   │   └── scripts/eval_radio_zero_shot.py  # RADIO 零样本评估
...
| **云端模型** | **C-RADIOv4 (7类水质)** | **C-RADIOv4 (3类场景)** | **C-RADIOv4 (4类场景)** |
| **云端F1** | 84.2% (坝体渗水) | 87.50% (总体) | 77.70% (总体) |
| **MQTT 联动** | 开启 | 开启 | 开启 |
| **训练数据** | YOLO 需要，C-RADIOv4 不需要 | YOLO 需要，C-RADIOv4 不需要 | YOLO 需要，C-RADIOv4 不需要 |
...
**云端零样本 (C-RADIOv4, 3 类, F1=87.50%)**:

| 类别 | 组合推理 | F1 |
|------|---------|-----|
| 消防通道堵塞 | 黄网格线 + 车辆/障碍物 | 73.33% |
| 翻越栏杆围墙 | 栏杆/围墙 + 人在其上 | **100.00%** |
| 安全出口堵塞 | 安全出口标识 + 障碍物 | 待验证 |

### 9.3 施工安全（15 检测 + 4 零样本）
...
**云端零样本 (C-RADIOv4, 4 类, F1=77.70%)**:

| 类别 | 视觉特征 | F1 |
|------|---------|-----|
| 裸土未覆盖 | 干燥黄褐色泥土纹理，无覆盖 | **83.67%** |
| 扬尘污染 | 空气中灰白色尘雾弥漫 | 76.00% |
| 坑内积水 | 水面反射/镜面光泽 | 73.47% |
| 基坑边材料堆放 | 基坑边缘堆放材料 | 待验证 |
