...
| 服务 | 技术栈 | 生产端口（Nginx 代理） | 开发端口（直连） | 说明 |
|------|--------|----------------------|-----------------|------|
...
> **生产模式**使用 Docker named volumes（`postgres_data`、`redis_data` 等），由 `deploy.sh --init` 自动管理，**无需手动创建目录**。
...
- **自动检测本机 IP** 写入 `CLOUD_API_URL`（无需手动设置）
- 自动检测 GPU 并决定是否启用 training 服务
...
| 配置项 | 默认值 | 说明 |
|--------|--------|------|
| `CLOUD_API_URL` | `http://192.168.0.103` | 后端外部访问地址（`deploy.sh --init` 自动检测） |
| `PG_PASSWORD` | `edge_pass` | 数据库密码（生产环境务必修改） |
| `EMQX_PASSWORD` | `admin123456` | EMQX 管理密码（`deploy.sh --init` 自动生成） |
| `API_KEY` | `edge-cloud-api-key-change-me` | API 认证密钥（`deploy.sh --init` 自动生成） |
...
| 命令 | 说明 |
|------|------|
| `./deploy.sh` | 生产部署（自动检测 GPU，一次性启动全部服务） |
| `./deploy.sh --rebuild` | 重新构建 backend/frontend 镜像并部署 |
| `./deploy.sh --stop` | 停止所有服务 |
| `./deploy.sh --status` | 查看服务状态 |
| `./deploy.sh --logs [service]` | 查看日志（默认后端） |
| `./deploy.sh --backup` | 备份 PostgreSQL 数据库 |
| `./deploy.sh --restore <file>` | 从备份恢复数据库 |
...
docker exec edge_cloud_training curl -s http://localhost:5002/health  # training
```
...
edge_cloud_frontend     Up
edge_cloud_nginx        Up (healthy)    0.0.0.0:80->80/tcp, 0.0.0.0:443->443/tcp
rtmp-server             Up              0.0.0.0:1935->1935/tcp
edge_cloud_training     Up (healthy)    # --profile gpu 时显示
```

### 6.4 服务访问地址

所有服务通过 Nginx 统一入口（`http://<服务器IP>/`）：

| 服务 | 地址 |
|------|------|
| 前端管理界面 | `http://<IP>/` |
| 后端 REST API | `http://<IP>/api/` |
| API 文档 | `http://<IP>/api/swagger-ui.html` |
| WebSocket | `http://<IP>/ws` |
...
生产模式下 REST API 需要认证。API Key 由 `deploy.sh --init` 自动生成并写入 `.env`：
...
## 7. 开发环境部署（Windows / Linux 本地开发）

> 使用 `docker-compose.yml`，各服务端口直接对外暴露，支持源码热重载和远程调试。
...
curl http://localhost:5002/health            # 训练服务（GPU）
```
...
2. **配置 API Key 认证**：`.env` 中设置强 API Key，第三方调用携带 `X-API-Key` header（`deploy.sh --init` 自动生成）
3. **配置 HTTPS**：在 `deployment/docker/nginx/prod.conf` 中添加 SSL 证书配置
4. **资源限制**：`docker-compose.prod.yml` 中已通过 `deploy.resources` 限制 CPU/内存
5. **定期备份**：`./deploy.sh --backup` 备份 PostgreSQL 数据
6. **日志管理**：`docker-compose.prod.yml` 中已配置日志轮转（50m × 5 文件）
7. **监控告警**：利用后端 `/actuator/health` 端点配置健康检查
8. **开机自启**：`deploy.sh --init` 自动安装 systemd 服务（路径自适应，不依赖仓库位置），管理命令：
   ```bash
   sudo systemctl start edge-cloud    # 启动
   sudo systemctl stop edge-cloud     # 停止
   sudo systemctl status edge-cloud   # 状态
   ```
...
> **API 地址约定**：以下 §13~§17 的 API 示例使用开发模式地址 `http://localhost:8081`。
> **生产环境**请替换为 `http://<服务器IP>`（Nginx 代理，端口 80，无 8081）。
...
# 一键部署（自动生成 .env、检测 IP 和 GPU、构建镜像、启动所有服务）
./deploy.sh
...
```powershell
# PowerShell（推荐）— 加 -it 使 Ctrl+C 可正常终止进程
...
```bash
# Git Bash（需加路径保护前缀）
MSYS_NO_PATHCONV=1 docker exec -it edge_cloud_training python3 /app/models/cloud_inference/radio_infer_server.py \
  --config /app/models/construction_safety/configs/construction_safety.yaml \
  --checkpoint /app/models/C-RADIOv4-H/c-radio_v4-h_half.pth.tar \
  --radio-code /app/models/NVlabs_RADIO \
  --siglip2 /app/models/siglip2-giant-opt-patch16-384
...
**生产环境**：浏览器打开 `http://192.168.0.103/emqx/`（通过 Nginx，账号见 `.env`）

**开发环境**：浏览器打开 `http://192.168.0.103:18083`（账号：admin / admin123456）
...
**生产环境**（API 地址 `http://192.168.0.103/api/`，无端口号）：

```bash
# 查询所有推理结果
curl -s http://192.168.0.103/api/v1/inference/results?page=1\&page_size=5 | python3 -m json.tool

# 告警查询
curl -s "http://192.168.0.103/api/v1/inference/alerts?levels=critical,warning" | python3 -m json.tool

# 统计信息
curl -s http://192.168.0.103/api/v1/inference/stats | python3 -m json.tool
```

**开发环境**（API 地址 `http://localhost:8081`）：
...
**生产环境**：前端 `http://192.168.0.103/alert-rules`

```bash
# 裸土未覆盖告警 (云端分割面积 > 5%)
curl -X POST http://192.168.0.103/api/v1/alert-rules \
  -H "Content-Type: application/json" \
...
**开发环境**：前端 `http://192.168.0.103:3000/alert-rules`
...
**生产环境**（`http://192.168.0.103`）：

```bash
# 导出 CSV (Excel 兼容)
curl "http://192.168.0.103/api/v1/inference/export?format=csv" -o inference_results.csv

# 按条件过滤导出
curl "http://192.168.0.103/api/v1/inference/export?source=cloud&alert_level=critical&format=csv" -o critical_cloud_alerts.csv
```

**开发环境**（`http://192.168.0.103:8081`）：
...
- 查看 API 文档：**生产** `http://<服务器IP>/api/swagger-ui.html`，**开发** `http://localhost:8081/swagger-ui.html`
